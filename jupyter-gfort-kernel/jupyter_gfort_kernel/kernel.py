...
class gfortKernel(Kernel):
    ...
    def __init__(self, *args, **kwargs):
        super(gfortKernel, self).__init__(*args, **kwargs)
        self.files = []
    ...
    def do_execute(self, code, silent, store_history=True,
        ...
        if magics['fig']:
            try: 
                ...
                self._write_to_stderr("[gfort kernel]{}".format(e))
            finally:
...
#                        binary_file.name, magics['fcflags'], magics['ldflags'])             # windows ifort
                ...
                with self.new_temp_file(suffix='.out', dir=tmpdir) as binary_file:           # Linux/WSL
                    ...
                    if p.returncode != 0:  # Compilation failed
                        self._write_to_stdout(
                            "[gfort kernel] fortran exited with code {}, the executable will not be executed".format(
                                    p.returncode))
                        return {'status': 'ok', 'execution_count': self.execution_count, 'payload': [],
                             'user_expressions': {}}
            if magics['module'] != []:
...
#                os.remove(src[-11:] + '.obj')                      # windows ifort
                ...
                self._write_to_stderr("[gfort kernel] module objects created successfully: {}".format(mod_name))
...
#            os.remove(exe_file)                                    # windows ifort               
        ...
        if p.returncode != 0:
            self._write_to_stderr("[gfort kernel] Executable exited with code {}".format(p.returncode))
        return {'status': 'ok', 'execution_count': self.execution_count, 'payload': [], 'user_expressions': {}}
